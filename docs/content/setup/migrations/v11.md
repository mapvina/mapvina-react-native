...
MapVina React Native v11 is the first release to only support the new architecture. As this required significant
changes to the implementation, the APIs were also improved and are based upon MapVina GL JS.

## Changes to `Map` Component

The `MapView` was renamed to `Map` to align with MapVina GL JS.
...
is [pre-dating MapVina](https://blog.mapbox.com/asynchronous-rendering-on-android-831722ac1837) and was never
...
- `localizeLabels`: This was used in conjunction with Mapbox specific map styles for toggling localization, MapVina
...
Interaction props have been aligned with the MapVina GL JS API as far as possible.
...
Imperative methods have been aligned with the MapVina GL JS API as far as possible.
...
The prop was renamed and the structure was aligned with MapVina GL JS and `react-map-gl`:
...
Tiled sources now use MapVina Style spec aligned props:
...
The `ShapeSource` component has been renamed to `GeoJSONSource` to align with MapVina GL JS terminology:
...
The separate `url` and `shape` props have been unified into a single `data`, to align with MapVina GL JS. The `data`
...
Several props have been renamed to align with the MapVina Style Spec and MapVina GL JS:
...
Several imperative methods have been renamed and simplified to align with MapVina GL JS:
...
Due to MapVina Native limitations this is not the original data but always a `FeatureCollection`. For example a
...
All cluster-related methods accept a `clusterId` number instead of a GeoJSON Feature to align with MapVina GL JS. The
...
The imperative method has been renamed and simplified to align with MapVina GL JS:
...
Several props have been renamed to align with MapVina Style spec:
...
- Type-safe autocomplete using `LayerSpecification` from `@mapvina/mapvina-gl-style-spec`
...
The `anchor` prop changed from a numeric `{ x, y }` object to a string anchor position aligned with MapVina GL JS:
...
This component only relies on the default MapVina Native user location implementations without any custom React Native
customizations.
...
```diff
const offlinePack = await OfflineManager.createPack({
-  styleURL: 'https://demotiles.mapvina.com/tiles/tiles.json',

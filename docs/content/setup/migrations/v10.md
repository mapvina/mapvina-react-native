...
Any namespace can be chosen, but using `MapVinaGL` is not recommended anymore due to risk of confusion.
...
```diff
- import { locationManager, offlineManager, snapshotManager } from "@mapvina/mapvina-react-native";
+ import { LocationManager, OfflineManager, SnapshotManager } from "@mapvina/mapvina-react-native";
```

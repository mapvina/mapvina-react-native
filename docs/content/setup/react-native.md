...
First install `@mapvina/mapvina-react-native` with the package manager of your choice, e.g. with `yarn`:

```bash
yarn add @mapvina/mapvina-react-native
```
...
You can customize the setup of MapVina React Native through `gradle.properties` on Android or global variables in the

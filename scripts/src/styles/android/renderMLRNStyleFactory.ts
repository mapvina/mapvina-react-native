...
const ANDROID_PROP_NAME_MAP: Record<string, string> = {
  iconOverlap: "iconAllowOverlap",
  textOverlap: "textAllowOverlap",
};

function androidPropName(name: string): string {
  return ANDROID_PROP_NAME_MAP[name] ?? name;
}
  ...
  const setterMethods = layers
    .flatMap((layer) =>
      layer.properties.flatMap((prop) => {
        ...
        layer.position = Position.fromPosition(values[0]!!, values[1]!!, values[2]!!)`;
        ...
        } else if (ANDROID_PROP_NAME_MAP[prop.name]) {
          body = `        set${pascalCase(ANDROID_PROP_NAME_MAP[prop.name]!)}(layer, styleValue)`;
          ...
          body = `        if (styleValue.isExpression()) {
            if (styleValue.isImageStringValue) {
                layer.setProperties(PropertyFactory.${androidPropName(prop.name)}(styleValue.getImageStringValue()))
            } else {
                layer.setProperties(PropertyFactory.${androidPropName(prop.name)}(styleValue.getExpression()))
            }
        } else {
            layer.setProperties(PropertyFactory.${androidPropName(prop.name)}(${androidGetConfigType(androidInputType(prop.type, prop.value), prop)}))
        }`;
        } else {
          body = `        if (styleValue.isExpression()) {
            layer.setProperties(PropertyFactory.${androidPropName(prop.name)}(styleValue.getExpression()))

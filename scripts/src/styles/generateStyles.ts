import { latest as mapvinaGlStyleSpec } from "@mapvina/mapvina-gl-style-spec/src/reference/latest";
...
import { render as renderMapVinaRNStyles } from "./ts/renderMapVinaRNStyles";
...
type LayerType = keyof (typeof mapvinaGlStyleSpec)["layer"]["type"]["values"];
type SdkSupport = {
...
const TEMPLATE_MAPPINGS = [
  {
    render: renderMapVinaRNStyles,
    templatePath: path.join(TEMPLATES_PATH, "renderMapVinaRNStyles.ts"),
    output: path.join(PACKAGE_SRC_PATH, "types", "MapVinaRNStyles.ts"),
  },
  {
...
export async function generateStyles() {
  ...
  function getPropertiesForLight() {
    const lightSpecProperties = mapvinaGlStyleSpec.light;
  ...
  function getPropertiesForLayer(layerType: LayerType) {
    const specPaintProperties = mapvinaGlStyleSpec[`paint_${layerType}`];
    const specLayoutProperties = mapvinaGlStyleSpec[`layout_${layerType}`];

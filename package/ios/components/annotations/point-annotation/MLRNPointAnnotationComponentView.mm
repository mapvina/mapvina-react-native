...
+ (BOOL)shouldBeRecycled {
  // Prevent Fabric from recycling this view - MapVina reparents annotation views
...
- (void)prepareView {
  ...
  // Note: MapVina will reparent _view when displaying the annotation,
...
- (void)mountChildComponentView:(UIView<RCTComponentViewProtocol> *)childComponentView
  ...
  if ([childComponentView isKindOfClass:[RCTViewComponentView class]]) {
    ...
    if ([contentView isKindOfClass:[MLRNCallout class]]) {
      ...
      // NOTE: We do NOT add Callout as a subview - MapVina will present it
      ...
      // we need to refresh so MapVina picks up the custom view
  ...
  // This allows children to move with _view when MapVina reparents it.
  // Callouts are NOT added as subviews - they're presented by MapVina on selection.
  ...
  // If annotation was added before children were mounted, MapVina would have

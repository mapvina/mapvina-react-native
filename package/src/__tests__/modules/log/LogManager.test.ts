import { LogManager } from "@mapvina/mapvina-react-native";
...
describe("LogManager", () => {
  ...
  describe("handleLog", () => {
    ...
    describe("default logging behavior", () => {
      test("logs error messages to console.error", () => {
        ...
        expect(console.error).toHaveBeenCalledWith(
          "MapVina Native [ERROR] [TestTag] Test error message",
        );
      });
      ...
      test("logs warn messages to console.warn when logLevel is warn", () => {
        ...
        expect(console.warn).toHaveBeenCalledWith(
          "MapVina Native [WARN] [TestTag] Test warning message",
        );
      });
      ...
      test("logs info messages to console.info when logLevel is info", () => {
        ...
        expect(console.info).toHaveBeenCalledWith(
          "MapVina Native [INFO] [TestTag] Test info message",
        );
      });
      ...
      test("logs debug messages to console.info when logLevel is debug", () => {
        ...
        expect(console.info).toHaveBeenCalledWith(
          "MapVina Native [DEBUG] [TestTag] Test debug message",
        );
      });
      ...
      test("logs verbose messages to console.info when logLevel is verbose", () => {
        ...
        expect(console.info).toHaveBeenCalledWith(
          "MapVina Native [VERBOSE] [TestTag] Test verbose message",
        );
      });
    });
    ...
    describe("custom log handler", () => {
      test("calls custom handler and proceeds with default logging when handler returns false", () => {
        ...
        expect(console.error).toHaveBeenCalledWith(
          "MapVina Native [ERROR] [TestTag] Test message",
        );
      });
    ...
    describe("effectiveLevel", () => {
      test("reduces cancelled HTTP request level from warn to info", () => {
        ...
        expect(console.warn).not.toHaveBeenCalled();
        expect(console.info).toHaveBeenCalledWith(
          "MapVina Native [INFO] [Mbgl-HttpRequest] Request failed due to a permanent error: Canceled; Request was aborted",
        );
      });
      ...
      test("does not reduce level for other warning messages", () => {
        ...
        expect(console.warn).toHaveBeenCalledWith(
          "MapVina Native [WARN] [Mbgl-HttpRequest] Some other warning",
        );
        expect(console.info).not.toHaveBeenCalled();
      });
      ...
      test("does not reduce level for cancelled request from different tag", () => {
        ...
        expect(console.warn).toHaveBeenCalledWith(
          "MapVina Native [WARN] [OtherTag] Request failed due to a permanent error: Canceled",
        );
      });
    });
  });
  ...
  describe("integration scenarios", () => {
    test("complete lifecycle: start, log, stop", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "MapVina Native [ERROR] [IntegrationTest] Integration test message",
      );

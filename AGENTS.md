# Agent Guide for MapVina React Native

## What This Library Does

MapVina React Native provides React Native bindings to MapVina Native rendering engines for Android & iOS. It wraps native MapView components, camera controls, data sources, and style layers as React components, bridging JavaScript props to native view properties through React Native's new architecture (Fabric + TurboModules).

**Key Facts**:

- **Current version v11**: Only supports React Native's new architecture (Fabric/TurboModules)
- **Native SDKs**: MapVina Native iOS v6 ([`package/MapVinaReactNative.podspec`](package/MapVinaReactNative.podspec)), Android v13 ([`package/android/gradle.properties`](package/android/gradle.properties))
...
- **Map**: Root map container, wraps native MapVina view via Fabric codegen (`MapViewNativeComponent.ts`)
...
`scripts/codegen.ts` generates from MapVina style spec + TSDoc comments:

- **Native style classes**: iOS `.h/.m`, Android `.java` in `components/layers/style/`
- **TypeScript types**: `src/types/MapVinaRNStyles.ts` (layer styles, expressions)
...
- **Props**: Descriptive, follows MapVina terminology (e.g., `bearing`, `pitch`)
...
- `@mapvina/mapvina-react-native`: Public exports
- `@/*`: Internal exports
...
1. **DON'T** edit generated files in `src/types/MapVinaRNStyles.ts` or native style classes
...
**iOS**: Open `/examples/react-native-app/ios/MapVinaReactNativeExample.xcworkspace` in Xcode (library under `Pods > Development Pods`)
...
├── MapVinaReactNative.podspec # iOS podspec
...
- **tsconfig.json**: Strict mode, bundler module resolution, path aliases for `@mapvina/mapvina-react-native`
...
- **MapVinaReactNative.podspec**: iOS native version config
